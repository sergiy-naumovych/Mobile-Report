...
/**
 * Description of AjaxTest
 *
 * @author SERGE
 */
final class AjaxTest {
    ...
    public static function requestType() {
        ...
        switch ($_POST['type']) {
            ...
            case 'login':
                $res['company']['serial'] = self::checkSerialNumber($_POST['serial'], $_POST['login'], $_POST['pass'], $_POST['uuid']);
                if(!$res['company']['serial']) {
                    $res['company']['info'] = true;
                    break;
                }
    ...
    public static function checkSerialNumber($serial, $login, $pass, $uuid){
        $serial = self::checkData($serial);
        $login = self::checkData($login);
        $pass  = self::checkData($pass);
        if($login == "demo" && $pass == "demo") return true;
       ...
       $status = false;
        ...
        if($result = Connection::$mysqli->query($sql)){
            ...
            while($tmp = $result->fetch_assoc()){
                $status = $tmp;
                if(($tmp['status'] == 2 && $tmp['uuid'] != $uuid) || $tmp['status'] == 3) {
                    $status = false;
                } elseif ($tmp['status'] == 1) {
                    self::changeSerialStatus(2, $uuid, $serial);
                } elseif ($tmp['status'] == 4 && !$tmp['uuid']){
                    //demo and empty uuid
                    self::changeSerialStatus(4, $uuid, $serial);
                } elseif ($tmp['status'] == 4 && ($tmp['uuid'] != $uuid)) {
                    $status = false;
                }
            }
            $result->free();
    ...
    public static function changeSerialStatus($status, $uuid, $serial){
         try {
